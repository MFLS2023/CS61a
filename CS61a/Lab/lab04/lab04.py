...
def merge(s, t):
    ...
    """
    "*** YOUR CODE HERE ***"
    if not s:  # 如果 s 为空，直接返回 t
        return t
    if not t:  # 如果 t 为空，直接返回 s
        return s
    if s[0] < t[0]:
        return [s[0]] + merge(s[1:], t)  # 递归合并
    else:
        return [t[0]] + merge(s, t[1:])  # 递归合并
...
def size_of_tree(t):
    ...
    if not t:
        ...
        size = 1  # 根节点的大小
        for b in branches(t):
            size += size_of_tree(b)  # 递归计算每个子树的大小
        return size
...
def divide(quotients, divisors):
    ...
    """
    return {quotients: d for d in divisors if d%quotients==0}
